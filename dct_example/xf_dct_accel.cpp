...
Xpose_Row_Outer_Loop:
  ...
  Xpose_Row_Inner_Loop:
    for (i = 0; i < DCT_SIZE; i++) col_inbuf[j][i] = row_outbuf[i][j];
  // DCT columns
Col_DCT_Loop:
  for (i = 0; i < DCT_SIZE; i++) {
...
Xpose_Col_Outer_Loop:
  ...
  Xpose_Col_Inner_Loop:
    for (i = 0; i < DCT_SIZE; i++) out_block[j][i] = col_outbuf[i][j];
}
...
RD_Loop_Row:
  ...
  RD_Loop_Col:
    for (c = 0; c < DCT_SIZE; c++) buf[r][c] = input[r * DCT_SIZE + c];
  }
}
...
WR_Loop_Row:
  for (r = 0; r < DCT_SIZE; r++) {
  WR_Loop_Col:
    for (c = 0; c < DCT_SIZE; c++) output[r * DCT_SIZE + c] = buf[r][c];
  }
}

void dct_accel(short input[N_DCT], short output[N_DCT]) {
